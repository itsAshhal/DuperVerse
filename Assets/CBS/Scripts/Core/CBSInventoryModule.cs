...
namespace CBS
{
    public class CBSInventoryModule : CBSModule, ICBSInventory
    {
        ...
        {
            var profileID = Profile.ProfileID;
            var instances = InventoryCache.Select(x => x.Value).Where(x => x.Type == ItemType.LOOT_BOXES).ToList();
        ...
        /// <param name="instanceId"></param>
        /// <param name="result"></param>
        public void OpenLootbox(string instanceId, Action<CBSOpenLootboxResult> result)
        {
            var profileID = Profile.ProfileID;
            FabInventory.UnlockContainer(profileID, instanceId, onOpen =>
            {
