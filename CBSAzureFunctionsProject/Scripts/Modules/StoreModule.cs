...
namespace CBS
{
    public class StoreModule : BaseAzureModule
    {
        ...
        public static async Task<dynamic> GetAllStoresTrigger([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger log)
        {
            ...
            var getResult = await GetStoresAsync(profileID, timeZone, log);
        ...
        public static async Task<dynamic> GetStoreByIDTrigger([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger log)
        {
            ...
            var getResult = await GetStoreByIDAsync(profileID, storeID, timeZone, log);
            if (getResult.Error != null)
            {
                return ErrorHandler.ThrowError(getResult.Error).AsFunctionResult();
            }
        ...
        public static async Task<ExecuteResult<FunctionGetStoresResult>> GetStoresAsync(string profileID, int timeZone, ILogger log)
        {
            // get profile detail
            var constraints = new CBSProfileConstraints
            {
            ...
            if (profileResult.Error != null)
            {
                return ErrorHandler.ThrowError<FunctionGetStoresResult>(profileResult.Error);
            }
            var profileDetail = profileResult.Result;
            ...
            foreach(var id in ids)
            {
                var storeTask = GetStoreWithProfileLimitsAsync(profileDetail, id, timeZone, log);
                taskList.Add(storeTask);
            }
        ...
        public static async Task<ExecuteResult<CBSStore>> GetStoreByIDAsync(string profileID, string storeID, int timeZone, ILogger log)
        {
            // get profile detail
            var constraints = new CBSProfileConstraints
            {
            ...
            var requestTime = ServerTimeUTC;
            requestTime.AddMilliseconds(timeZone);
            var storeResult = await GetStoreWithProfileLimitsAsync(profileDetail, storeID, timeZone, log);
        ...
        {
            var storeResult = await GetStoreByIDAsync(profileID, storeID, timeZone, log);
            if (storeResult.Error != null)
            {
        ...
        public static async Task<ExecuteResult<CBSStore>> GetStoreWithProfileLimitsAsync(ProfileEntity profileDetail, string storeID, int timeZone, ILogger log)
        {
            var profileID = profileDetail.ProfileID;
